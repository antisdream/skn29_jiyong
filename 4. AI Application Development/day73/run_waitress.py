...
class StaticAndMediaFilesHandler(StaticFilesHandler):
    def __init__(self, application):
        ...
        self.media_path = self.media_url.path
        if self.media_path and not self.media_path.startswith('/'):
            self.media_path = '/' + self.media_path
    ...
    def _should_handle(self, path):
        ...
        return super()._should_handle(path) or (
            bool(self.media_path) and path.startswith(self.media_path) and not self.media_url.netloc
        )

    def serve(self, request):
        # 미디어 파일 요청인 경우 미디어 루트에서 찾아 서빙
        if self.media_path and request.path.startswith(self.media_path):
            path = request.path[len(self.media_path):]
